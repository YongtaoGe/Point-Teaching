...
class MIL_FCOSOutputs_v3(nn.Module):
    def __init__(self, cfg):
        super(MIL_FCOSOutputs_v3, self).__init__()
    ...
    def fcos_losses(self, instances):
        ...
        for k, v in losses.items():
            losses[k] = torch.where(torch.isnan(losses[k]), torch.zeros_like(losses[k]), losses[k] * 0.0)
