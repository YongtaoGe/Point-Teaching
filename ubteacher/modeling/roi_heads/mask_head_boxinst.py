...
class BoxSupMaskRCNNConvUpsampleHead(MaskRCNNConvUpsampleHead):
    @configurable
    def __init__(self, 
    ...
    def forward(self, x, instances: List[Instances], branch=''):
        ...
        if self.training and (branch=='supervised' or branch=='pseudo_supervised'): 
            ...
            if self.boxinst_enabled:
                ...
                #     pdb.set_trace()
                # print(x.size())
                loss.update(self.boxinst_loss(x, instances, branch))
                # import pdb
                # pdb.set_trace()
    ...
    @torch.jit.unused
    def boxinst_loss(self, pred_mask_logits: torch.Tensor, instances: List[Instances], branch=''):
        ...
        for instances_per_image in instances:
            ...
            gt_image_color_similarity_per_image = crop_and_resize_image_color_similarity(
                instances_per_image.gt_image_color_similarity, 
                instances_per_image.proposal_boxes.tensor, 
                mask_side_len,
                1.0 / self.mask_out_stride)

            gt_bitmasks.append(gt_bitmasks_per_image)    
            gt_image_color_similarity.append(gt_image_color_similarity_per_image)
        ...
        # if gt_bitmasks.min()==0:
        #     import pdb
        #     pdb.set_trace()
        weights = (gt_image_color_similarity >= self.pairwise_color_thresh).float() * gt_bitmasks
