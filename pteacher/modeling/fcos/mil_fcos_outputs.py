...
from pteacher.utils.comm import reduce_sum, reduce_mean, compute_ious
from pteacher.layers import ml_nms, IOULoss
...
class MIL_FCOSOutputs(nn.Module):
    def __init__(self, cfg):
        super(MIL_FCOSOutputs, self).__init__()
    ...
    def losses(self, ins_mil_logits_pred, logits_pred, reg_pred, ctrness_pred, locations, gt_instances, top_feats=None):
        """
        ...
        if self.ins_mil_loss_weight > 0:
            instances.ins_mil_logits_pred = cat([
                # Reshape: (N, C, Hi, Wi) -> (N, Hi, Wi, C) -> (N*Hi*Wi, C)
    ...
    def fcos_losses(self, instances):
        ...
        all_ins_mil_logits_pred = instances.ins_mil_logits_pred
        ...
        if self.ins_mil_loss_weight > 0:
            # mil_losses = []
            # pos_assigned_gt_labels = labels[pos_inds]
            # pos_assigned_gt_inds = instances.gt_inds  # (#p,)
            # pos_mil_filters = instances.ins_mil_logits_pred  # (#p, 2)
            # pos_cls_scores = instances.logits_pred # (#p, num_classes)
            # # pos_centerness_targets = instance_im.logits_pred[pos_inds_im] # (#p,)

            # for g_id in torch.unique(pos_assigned_gt_inds):
            #     pos_mil_inst = pos_mil_filters[pos_assigned_gt_inds == g_id]  # (#p1, 2)
            #     # pos_centerness_inst = pos_centerness_targets[pos_assigned_gt_inds == g_id]  # (#p1,)
            #     cls_label = torch.unique(pos_assigned_gt_labels[pos_assigned_gt_inds == g_id])
            #     pos_cls_inst = pos_cls_scores[pos_assigned_gt_inds == g_id]

            #     log_pos_cls_inst = F.log_softmax(pos_cls_inst, dim=1)[:, cls_label]  # (#p1,)
            #     log_prob = F.log_softmax(pos_mil_inst, dim=1)
            #     log_fg_prob = (log_prob[:, 1] + log_pos_cls_inst.detach())
            #     log_bg_prob = log_prob[:, 0]
            #     eye = torch.eye(len(pos_mil_inst), dtype=torch.float32, device=log_fg_prob.device)
            #     log_prob_prod = (eye * log_fg_prob[None, :] + (1 - eye) * log_bg_prob[None, :]).sum(dim=-1)  # (#p1,)
            #     max_ = log_prob_prod.max()
            #     final_log_prob = torch.log(torch.exp(log_prob_prod - max_).sum()) + max_
            #     mil_losses.append(-final_log_prob)

            mil_losses = []
            if len(instances) == 0:
                bs = 0
            else:
                bs = instances.im_inds.max() + 1
            
            for im_id in range(bs):
                instance_im = instances[instances.im_inds == im_id]
                labels_im = instance_im.labels.flatten()
                pos_inds_im = torch.nonzero(labels_im != num_classes).squeeze(1)
                # pos_inds_im = (assigned_gt_inds_im >= 0).nonzero().reshape(-1)
                if len(pos_inds_im) == 0:
                    continue
                pos_assigned_gt_labels = labels_im[pos_inds_im]
                pos_assigned_gt_inds = instance_im.gt_inds[pos_inds_im]  # (#p,)
                pos_mil_filters = instance_im.ins_mil_logits_pred[pos_inds_im]  # (#p, 2)
                pos_cls_scores = instance_im.logits_pred[pos_inds_im] # (#p, num_classes)
                # pos_centerness_targets = instance_im.logits_pred[pos_inds_im] # (#p,)
                ...
                for g_id in torch.unique(pos_assigned_gt_inds):
                    ...
                    log_pos_cls_inst = F.log_softmax(pos_cls_inst, dim=1)[:, cls_label]  # (#p1,)
                    log_prob = F.log_softmax(pos_mil_inst, dim=1)
                    # if self.mil_with_cls:
                    #     log_fg_prob = (log_prob[:, 1] + torch.log(pos_centerness_inst) + torch.log(pos_cls_inst))
                    # else:
                    #     log_fg_prob = (log_prob[:, 1] + torch.log(pos_centerness_inst))  # (#p1,)

                    # log_fg_prob = (log_prob[:, 1] + torch.log(pos_cls_inst))   
                    log_fg_prob = (log_prob[:, 1] + log_pos_cls_inst.detach())
                    log_bg_prob = log_prob[:, 0]
                    eye = torch.eye(len(pos_mil_inst), dtype=torch.float32, device=log_fg_prob.device)
                    log_prob_prod = (eye * log_fg_prob[None, :] + (1 - eye) * log_bg_prob[None, :]).sum(dim=-1)  # (#p1,)
            ...
            if len(mil_losses) > 0:
                mil_loss = sum(mil_losses) / num_pos_avg
                # print(mil_loss)
            else:
                mil_loss = instances.ins_mil_logits_pred.sum() * 0.
            losses["loss_ins_mil"] = mil_loss
    ...
    def predict_proposals(
            ...
            locations, image_sizes, top_feats=None, ins_mil_logits_pred=None, branch="supervised"
    ):
        if self.training:
        ...
        if len(ins_mil_logits_pred) > 0:
            bundle["ins_mil"] = ins_mil_logits_pred
        ...
        for i, per_bundle in enumerate(zip(*bundle.values())):
            ...
            ins_mil = per_bundle["ins_mil"] if "ins_mil" in bundle else None

            sampled_boxes.append(
                self.forward_for_single_feature_map(
                    l, o, r, c, image_sizes, top_feat=t, ins_mil_logits_pred=ins_mil, branch=branch
                )
            )
    ...
    def forward_for_single_feature_map(
            ...
            ctrness_pred, image_sizes, top_feat=None, img_mil_logits_pred=None, ins_mil_logits_pred=None, branch="supervised"
    ):
        ...
        if img_mil_logits_pred is not None:
            img_mil_logits_pred = img_mil_logits_pred.view(N, -1, H, W).permute(0, 2, 3, 1)
            img_mil_logits_pred = img_mil_logits_pred.reshape(N, H * W, -1)
        if ins_mil_logits_pred is not None:
            ins_mil_logits_pred = ins_mil_logits_pred.view(N, -1, H, W).permute(0, 2, 3, 1)
            ins_mil_logits_pred = ins_mil_logits_pred.reshape(N, H * W, -1)
        ...
        for i in range(N):
            ...
            if top_feat is not None:
                per_top_feat = top_feat[i][per_box_loc]
            if img_mil_logits_pred is not None:
                per_img_mil_logits_pred = img_mil_logits_pred[i][per_box_loc]
            if ins_mil_logits_pred is not None:
                per_ins_mil_logits_pred = ins_mil_logits_pred[i][per_box_loc]
            if img_mil_logits_pred is not None and ins_mil_logits_pred is not None:
                per_cls_feat = logits_pred[i][per_box_loc]
            ...
            if per_candidate_inds.sum().item() > per_pre_nms_top_n.item():
                ...
                if img_mil_logits_pred is not None and ins_mil_logits_pred is not None:
                    per_cls_feat = per_cls_feat[top_k_indices]
            ...
            if branch == "unsup_data_weak" and ins_mil_logits_pred is not None and self.ins_mil_loss_weight > 0:
                boxlist.scores = torch.sqrt(F.softmax(per_ins_mil_logits_pred, dim=-1)[..., 1] * torch.sqrt(per_box_cls))
            else:
            ...
            if img_mil_logits_pred is not None:
                boxlist.img_mil_logits_pred = per_img_mil_logits_pred
            if ins_mil_logits_pred is not None:
                boxlist.ins_mil_logits_pred = per_ins_mil_logits_pred
            if img_mil_logits_pred is not None and ins_mil_logits_pred is not None:
                boxlist.pre_cls_feat = per_cls_feat
                # import pdb
                # pdb.set_trace()
    ...
    def select_over_all_levels(self, boxlists, branch="normal"):
        num_images = len(boxlists)
        results = []
        for i in range(num_images):
            # multiclass nms
            if branch == "point_proposals":
                result = ml_nms(boxlists[i], 0.9)
            else:
    ...
    def predict_point_proposals(
            self, img_mil_logits_pred, ins_mil_logits_pred, logits_pred, reg_pred, ctrness_pred,
            locations, image_sizes, top_feats=None, branch="supervised"
    ):
        # if self.training:
        ...
        bundle = {
            "l": locations,
            "img_mil": img_mil_logits_pred,
            "ins_mil": ins_mil_logits_pred,
            "o": logits_pred,
            "r": reg_pred,
            "c": ctrness_pred,
            "s": self.strides,
        ...
        for i, per_bundle in enumerate(zip(*bundle.values())):
            ...
            # we denormalize them here.
            l = per_bundle["l"]
            img_mil = per_bundle["img_mil"]
            ins_mil = per_bundle["ins_mil"]
            o = per_bundle["o"]
            r = per_bundle["r"] * per_bundle["s"]
            c = per_bundle["c"]
            t = per_bundle["t"] if "t" in bundle else None
            ...
            sampled_boxes.append(
                self.forward_for_single_feature_map(
                    l, o, r, c, image_sizes, t, img_mil, ins_mil, branch
                )
            )
        ...
        boxlists = list(zip(*sampled_boxes))
        boxlists = [Instances.cat(boxlist) for boxlist in boxlists]
        boxlists = self.select_over_all_levels(boxlists, "point_proposals")

        return boxlists

    def predict_probs_img(self, proposals: List[Instances]):

        pred_img_mil_logits = []
        for proposals_per_img in proposals:
            pred_img_mil_logits.append(
                F.softmax(proposals_per_img.logits_pred, dim=1) * F.softmax(
                    proposals_per_img.img_mil_logits_pred, dim=0)
            )

        pred_class_img_logits = cat(
            [
                torch.sum(xx, dim=0, keepdim=True)
                for xx in pred_img_mil_logits
            ],
            dim=0,
        )
    ...
    def binary_cross_entropy_loss(self, point_proposals, gt_classes_img_oh, reduction="mean"):
        """
        ...
        return F.binary_cross_entropy(
            self.predict_probs_img(point_proposals), gt_classes_img_oh, reduction=reduction
        ) / gt_classes_img_oh.size(0)

    def instance_mil_loss(self, all_proposals, point_proposals, gt_point_coords, gt_point_classes):

        pred_class_logits_list = [F.log_softmax(instance.logits_pred, dim=1) for instance in point_proposals]
        pred_pss_logits_list = [F.log_softmax(instance.ins_mil_logits_pred, dim=1) for instance in point_proposals]
        proposal_boxes_list = [instance.proposal_boxes.tensor for instance in point_proposals]
        anchor_boxes_list = [instance.anchors.tensor for instance in point_proposals]

        loss_pss = []
        for idx, (gt_point_coords_per_img, gt_point_classes_per_img, proposal_boxes_per_img, anchor_boxes_per_img, pred_class_logits_per_img,
                  pred_pss_logits_per_img) in enumerate(zip(
                gt_point_coords, gt_point_classes, proposal_boxes_list, anchor_boxes_list, pred_class_logits_list, pred_pss_logits_list)):
            if len(proposal_boxes_per_img) == 0 and len(anchor_boxes_per_img) == 0:
                # print('688 point without proposals')
                # print(all_proposals[idx].ins_mil_logits_pred.size(), point_proposals[idx].ins_mil_logits_pred.size(), gt_point_coords_per_img.size())
                loss_pss.append(point_proposals[idx].ins_mil_logits_pred.sum() * 0.)
                continue

            for point, point_label in zip(gt_point_coords_per_img, gt_point_classes_per_img):
                inside_idxs_p = (
                            ((point[0] >= proposal_boxes_per_img[:, 0]) & (point[0] <= proposal_boxes_per_img[:, 2]) & \
                            (point[1] >= proposal_boxes_per_img[:, 1]) & (point[1] <= proposal_boxes_per_img[:, 3])) | \
                            ((point[0] >= anchor_boxes_per_img[:, 0]) & (point[0] <= anchor_boxes_per_img[:, 2]) & \
                            (point[1] >= anchor_boxes_per_img[:, 1]) & (point[1] <= anchor_boxes_per_img[:, 3])) \
                ).nonzero().reshape(-1)

                # print(point_label)
                # assert point_label.max() == point_label.min()
                # point_label = point_label[0]
                # print(inside_idxs_p.size())
        ...
        if len(loss_pss) == 0:
            # import pdb
            # pdb.set_trace()
            print("len(loss_pss)", len(loss_pss))
            loss_pss.append(Instances.cat(point_proposals).ins_mil_logits_pred.sum() * 0.)
    ...
    def mil_losses(self, all_proposals, point_proposals, gt_classes_img_oh, gt_point_coords, gt_point_classes):
        assert (self.img_mil_loss_weight > 0) or (self.ins_mil_loss_weight > 0)
        loss = {}
        if self.img_mil_loss_weight > 0:
            loss['loss_img_mil'] = self.binary_cross_entropy_loss(all_proposals, gt_classes_img_oh)
        if self.ins_mil_loss_weight > 0:
            loss['loss_ins_mil'] = self.instance_mil_loss(all_proposals, point_proposals, gt_point_coords,
                                                          gt_point_classes)
