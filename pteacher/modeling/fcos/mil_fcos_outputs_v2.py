...
class MIL_FCOSOutputs(nn.Module):
        ...
        for i, per_bundle in enumerate(zip(*bundle.values())):
            ...
            sampled_boxes.append(
                self.forward_for_single_feature_map(
                    l, o, r, c, image_sizes, top_feat=t
                )
            )
    ...
    def predict_rpn_proposals(
        ...
        pred_reg_deltas: List[torch.Tensor],
        ...
        N = pred_reg_deltas[0].shape[0]
        pred_proposals = self._decode_proposals(locations, pred_reg_deltas)
        ...
        return find_top_rpn_proposals(
            ...
            image_sizes,
            nms_thresh,
            pre_nms_topk[self.training],
            post_nms_topk[self.training],
            min_box_size,
            self.training,
        )

    def _decode_proposals(self, locations, pred_reg_deltas: List[torch.Tensor]):
        """
        ...
        for locations_i, pred_reg_deltas_i, stride in zip(locations, pred_reg_deltas, self.strides):
            locations_i = cat([locations_i] * N, dim=0) # N*H*W, 2
            # N C H W-> N H W C -> N*H*W, 4
            pred_reg_deltas_i = pred_reg_deltas_i.permute(0, 2, 3, 1).reshape(-1, 4)
            proposals_i = torch.stack([
                locations_i[:, 0] - pred_reg_deltas_i[:, 0] * stride,
                locations_i[:, 1] - pred_reg_deltas_i[:, 1] * stride,
                locations_i[:, 0] + pred_reg_deltas_i[:, 2] * stride,
                locations_i[:, 1] + pred_reg_deltas_i[:, 3] * stride,
            ], dim=1)
            # Append feature map proposals with shape (N, Hi*Wi, B)
