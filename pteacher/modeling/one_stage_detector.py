...
from pteacher.modeling.meta_arch.rcnn import PseudoLabProposalNetwork
...
@META_ARCH_REGISTRY.register()
class MILOneStageDetector(PseudoLabProposalNetwork):
    """
    Same as :class:`detectron2.modeling.ProposalNetwork`.
    Uses "instances" as the return key instead of using "proposal".
    """
    def forward(self, batched_inputs, branch="supervised"):
        if self.training:
            return super().forward(batched_inputs, branch)
        processed_results = super().forward(batched_inputs, branch)
        processed_results = [{"instances": r["proposals"]} for r in processed_results]
        return processed_results
