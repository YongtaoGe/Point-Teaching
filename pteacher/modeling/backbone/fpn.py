...
class PseudoLabFPN(Backbone):
    ...
    def __init__(
        ...
        """
        super(PseudoLabFPN, self).__init__()
    ...
    def forward(self, x):
        ...
        # import pdb
        # pdb.set_trace()
        out_dict = {f: res for f, res in zip(self._out_features, results)}
        out_dict.update(bottom_up_features)
    ...
    def output_shape(self):
        ...
        # hack
        _in_feature_strides = {'res2': 4, 'res3': 8, 'res4': 16, 'res5': 32}
        _in_feature_channels = {'res2': 256, 'res3': 512, 'res4': 1024, 'res5': 2048}

        res.update({
            name: ShapeSpec(
                channels=_in_feature_channels[name], stride=_in_feature_strides[name]
            )
...
@BACKBONE_REGISTRY.register()
def build_pseudolab_resnet_fpn_backbone(cfg, input_shape: ShapeSpec):
    """
    Args:
        cfg: a detectron2 CfgNode
    Returns:
